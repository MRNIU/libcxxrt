...
int32_t vsprintf(char *buf, const char *fmt, va_list args) {
  ...
  repeat:
    ...
    case 'o': {
      str =
          number(str, va_arg(args, uint32_t), 8, field_width, precision, flags);
      break;
    }
    // 如果格式转换符是
    ...
    case 'p': {
      ...
      str = number(str, (uint32_t)(long)va_arg(args, void *), 16, field_width,
                   precision, flags);
      break;
    }
    // 若格式转换指示是 'x'  或
    ...
    case 'X': {
      str = number(str, va_arg(args, uint32_t), 16, field_width, precision,
                   flags);
      break;
    }
    ...
    case 'u': {
      str = number(str, va_arg(args, uint32_t), 10, field_width, precision,
                   flags);
      break;
    }
    // 若格式转换指示符是
